...
SOM网络也是竞争神经网络，但与上面讲到的有些许的不同。他引入了邻域函数的概念。在更新神经元时不止更新距离输入最近的神经元，还更新了它周围的神经元。这样可以保持一定的输入数据的拓扑结构。这个邻域半径会随着训练的进行不断地缩小。使得最后神经元都会像每一类靠拢。



![SOM网络训练过程](/images/2022-08-13/v2-f52b70ceb4be67a91ebd807236a27fbf_b.webp)
...
级联相关网络属于一种**结构自适应神经网络**，上面提到的竞争神经网络中的ART网络也属于这一类。在训练过程中不断增加新的神经元，使得神经元的输出与网络误差相关性最大来达到训练的目的。网络训练速度较快，但在数据量少的时候容易陷入过拟合。
...
另一种神经网络将神经元的输出加入下一时刻的输入，构成了**递归神经网络**。于是一个时间节点的网络输出会影响下一个节点的网络输出。这是一般前馈网络无法做到的。
...
在神经网络模型中增加隐藏层数，这就变成了深度神经网络。深度神经网络在如今广泛应用。主要有应用于图像处理的CNN，应用的自然语言处理的LSTM等。残差模块的引入使得深度神经网络的深度可以再度加强。孪生网络的概念引入为神经网络的判别任务提供了新方案，而生成对抗网络让神经网络也可以向人类一样作画、生成超分辨率，进行图像风格迁移等任务。

目前比较流行的是预训练大模型微调的任务。同时也有调整与处理输入数据让网络的表现更好的诸多尝试。
...
[7] https://zhuanlan.zhihu.com/p/73534694
